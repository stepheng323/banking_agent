...
def _normalized_prompt_tokens(value: str | None) -> list[str]:
    if not value:
        return []
...
def _relationship_display_from_tokens(tokens: list[str]) -> str | None:
    ...
    for alias in get_cached_guardrails().transfer.relational_aliases:
        alias_tokens = _normalized_prompt_tokens(alias)
        if alias_tokens:
...
def sanitize_recipient_display_name(recipient_name: str | None, locale: str = "en") -> str:
    ...
    tokens = _normalized_prompt_tokens(recipient_name)
    if not tokens:
        return fallback
...
def _possessive_display_name(display_name: str, locale: str = "en") -> str:
    if locale != "en":
        return display_name
    ...
    if ask_lines:
        ...
        if prefix:
            if needs_account_and_bank and locale == "en":
                ask_lines = [
                    f"I still need {_possessive_display_name(focused_display_name, locale)} account number and bank."
                ]
            return f"{prefix}\n\n" + "\n".join(ask_lines)
        return "\n".join(ask_lines)
