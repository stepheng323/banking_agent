...
from apps.chat.src.agent.graphs.__shared__.models.smart_context import SmartContext
from apps.chat.src.agent.graphs.data.models_extraction import DataExtractionResult
from apps.chat.src.agent.graphs.data.prompt import DATA_EXTRACTION_PROMPT
...
class DataEntityExtractor:
    ...
    def _context_mode(smart_context: dict[str, Any] | None) -> str:
        ...
        if smart_context.get("previousResponse"):
            return "compact"
        if any(smart_context.get(key) for key in ("target_phone", "network", "plan_name", "amount")):
            return "compact"
        return "full"
    ...
    def _build_context_string(self, smart_context: dict[str, Any] | None) -> str:
        ...
        if smart_context.get("target_phone"):
            parts.append(f"KnownTargetPhone: {smart_context['target_phone']}")
        if smart_context.get("network"):
            parts.append(f"KnownNetwork: {smart_context['network']}")
        if smart_context.get("plan_name"):
            parts.append(f"KnownPlan: {smart_context['plan_name']}")
        if smart_context.get("amount"):
            parts.append(f"KnownBudget: {smart_context['amount']}")
