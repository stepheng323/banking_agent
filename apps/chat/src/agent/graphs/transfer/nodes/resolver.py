...
def _referent_item_data(item: dict[str, Any] | None) -> dict[str, Any]:
...
def _beneficiary_record_from_referent_item(item: dict[str, Any]) -> dict[str, Any]:
    data = _referent_item_data(item)
    return {
...
def _memory_recipient_resolution(ctx: TransferContext) -> dict[str, Any] | None:
    candidate = ctx.resolved_referents.get("recipient")
    return candidate if isinstance(candidate, dict) else None


def _build_memory_recipient_clarify_result(resolution: dict[str, Any], locale: str) -> TransactionResult:
    ...
    for idx, item in enumerate(candidates[:5], start=1):
        ...
        record = _beneficiary_record_from_referent_item(item)
        candidate_list.append(
            {
                "index": idx,
                "option_id": option_id,
...
async def _resolve_memory_recipient_result(
    payload: TransferPayload,
    ctx: TransferContext,
    locale: str,
    resolver_provider: Any | None,
    ...
    resolution = _memory_recipient_resolution(ctx)
    if not resolution:
        return None
    status = str(resolution.get("status") or "").strip().lower()
    if status == "ambiguous":
        return _build_memory_recipient_clarify_result(resolution, locale)
    if status != "resolved":
        return None
    ...
    return await _saved_beneficiary_result(
        _build_single_beneficiary_patch_from_record(record, None),
        payload,
        locale,
        resolver_provider,
        bank_cache,
...
async def resolve_beneficiary(
    ...
    if _is_pronoun_recipient(raw_recipient_name) or (
        previous_reference and str(previous_reference.get("selector") or "").strip().lower() == "previous"
    ):
        memory_result = await _resolve_memory_recipient_result(payload, ctx, locale, resolver_provider, bank_cache)
