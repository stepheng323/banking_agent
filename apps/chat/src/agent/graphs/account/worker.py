...
from apps.chat.src.agent.graphs.account.capabilities import (
    AccountCapability,
    check_capabilities,
    derive_requirements,
...
from apps.chat.src.agent.graphs.account.parser import AccountParser
...
class AccountWorker:
    ...
    async def run(
        ...
        if action == "count":
            ...
            return AccountResult(
                ...
                response=response,
                patch=patch,
                details={"viewed_accounts": self._serialize_accounts(accounts)} if accounts else {},
            )
        ...
        try:
            if action == "link":
                ...
                response, viewed_accounts = await self._check_balance(user_id, account_identifiers, locale=locale)
            else:
    ...
    async def _check_balance(
        ...
        if not balances:
            return render_message("account.balance.unavailable", locale), self._serialize_accounts(target_accounts)
