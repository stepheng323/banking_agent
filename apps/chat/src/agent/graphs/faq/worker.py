...
Stateless worker for FAQ tasks using LangGraph.
"""

from typing import Any

from apps.chat.src.agent.graphs.faq.nodes.gate import confidence_gate_node
from apps.chat.src.agent.graphs.faq.nodes.guard import final_guard_node
from apps.chat.src.agent.graphs.faq.nodes.normalize import normalize_query_node
from apps.chat.src.agent.graphs.faq.nodes.retrieve import create_retrieve_node
from apps.chat.src.agent.graphs.faq.nodes.synthesize import create_synthesize_node
...
class FAQWorker:
    ...
    async def run(
        ...
        try:
            ...
            state = final_guard_node(state)

            return FAQResult(
                outcome=FAQOutcome.OK,
                response=state.get("response", ""),
                should_route_to_support=state.get("should_route_to_support", False),
            )
