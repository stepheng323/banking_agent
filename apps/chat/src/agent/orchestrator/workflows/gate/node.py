...
from apps.chat.src.agent.orchestrator.workflows.gate.engine import run_gate_engine
from apps.chat.src.agent.orchestrator.workflows.gate.registry import GATE_HANDLER_SPECS
...
async def session_gate_direct_path(state: OrchestratorState, config: RunnableConfig) -> dict[str, Any]:
    ...
    result = await run_gate_engine(ctx, GATE_HANDLER_SPECS)
