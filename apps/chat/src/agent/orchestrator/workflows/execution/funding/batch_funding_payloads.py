...
def _build_planning_signature(task_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "planned_for_amount": naira_to_json(task_payload.get("amount")) or "0.00",
...
def _funding_plan_to_payload_dict(plan: Any, task_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "transfer_amount": naira_to_json(plan.transfer_amount),
        "total_funded": naira_to_json(plan.total_funded),
        ...
        "primary_available_balance": naira_to_json(plan.primary_available_balance),
