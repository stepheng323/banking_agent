...
async def advance_wave(state: OrchestratorState, config: RunnableConfig) -> dict[str, Any]:
    ...
    for task_id in current_wave:
        ...
        # During an input interrupt turn, keep same-domain sibling tasks paused so a transfer
        # slot reply cannot drift another transfer. Different transaction domains can still
        # catch up because handlers withhold the focused user message from non-active tasks.
        active_input_task_types = set()
        if state.last_interrupt:
            active_input_task_types = {
        ...
        if task.type in TRANSACTION_TASK_TYPES:
            propagated_task_ids = _propagate_batch_source_selection(
                state=state,
                selected_task_id=task_id,
                locale=locale,
            )
            for propagated_task_id in propagated_task_ids:
                ...
                if propagated_task and propagated_task.stage == TaskStage.AWAITING_CONFIRMATION:
                    agg.needs_confirm_tasks.append(propagated_task_id)
        progressed = True
    ...
    if agg.needs_confirm_tasks:
        confirm_task_ids = _gate_task_ids(
            state=state,
            current_wave=current_wave,
