"""Redis-backed orchestration task-state service."""
...
class TaskStateService:
    """Service for managing planned task state in Redis."""
    ...
    async def create_task_queue(self, phone_number: str, planner_output: PlannerOutput) -> None:
        """
        Store planned task state from planner output.
    ...
    async def get_task_queue(self, phone_number: str) -> PlannerOutput | None:
        """
        Retrieve current planned task state.

        Args:
            phone_number: User's phone number

        Returns:
            PlannerOutput if task state exists, None otherwise
        """
        queue_key = f"user:{phone_number}:task_queue"
    ...
    async def clear_task_queue(self, phone_number: str) -> None:
        """
        Clear completed/failed task state.
    ...
    async def has_active_queue(self, phone_number: str) -> bool:
        """
        Check if user has active planned task state.
