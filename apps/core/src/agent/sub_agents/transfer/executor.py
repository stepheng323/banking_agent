...
class TransferExecutor:
    ...
    async def handle_transfer(self, transfer_request: dict[str, Any]) -> None:
        ...
        try:
            provider = PaymentProviderFactory.get_provider_for_service("initiate_transfer")
            if not provider:
            ...
            transfer_result = await provider.initiate_transfer(
                ...
                sender_account_number=transfer_data.get("source", {}).get("account_number"),
