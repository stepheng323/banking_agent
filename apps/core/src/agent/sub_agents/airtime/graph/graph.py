...
CONTINUATION_PHRASES = {
    "yes", "yep", "yeah", "ok", "okay", "sure", "continue", "proceed",
    "go ahead", "confirm", "yes continue", "yes please", "go on",
    "that's correct", "thats correct", "correct", "right", "yes confirm"
}
...
class AirtimeFlowGraph:
    ...
    async def _handle_flow_resume(self, phone_number: str) -> Optional[str]:
    ...
    async def _load_state_from_checkpoint(self, config: RunnableConfig) -> tuple[Optional[dict], bool]:
        ...
        try:
            ...
            if current_state and current_state.values:
                ...
                # Check if stale
                if flow_state in ("cancelled", "completed", "error") or \
                   airtime_status in ("completed", "failed", "cancelled"):
                    return None, True
                
                logger.info("checkpoint_loaded", flow_state=flow_state, amount=loaded_state.get("amount"))
    ...
    async def _load_state_from_redis(
        self, phone_number: str, message: str, message_id: str, classification_result: Optional[dict]
    ) -> Optional[dict]:
    ...
    async def _handle_mid_correction(
        ...
        input_state: dict, classification_result: Optional[dict],
        ...
        if not changed:
            ...
            is_continuation = any(p in msg_lower for p in CONTINUATION_PHRASES) or msg_lower in CONTINUATION_PHRASES
            
            if is_continuation:
                logger.info("continuation_detected", message=message[:20])
                new_state = create_initial_state(phone_number, message, message_id, classification_result)
                for k, v in preserved.items():
                    if v is not None:
                        new_state[k] = v
            ...
            clarification_state = create_initial_state(phone_number, message, message_id, classification_result)
        ...
        if old_values["recipient_phone"] != new_vals["recipient_phone"] and new_vals["recipient_phone"]:
    ...
    async def run(
        ...
        logger.info("airtime_flow_completed",
    ...
    async def resume_after_pin_verification(
        ...
        logger.info("pin_resume_completed",
                   flow_state=final_state.get("flow_state"),
                   airtime_status=final_state.get("airtime_status"))
