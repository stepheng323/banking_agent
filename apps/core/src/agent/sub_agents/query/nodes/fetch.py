...
from apps.core.src.agent.sub_agents.query.prices import is_variable_item, lookup_item_price
...
async def fetch_balance(state: QueryState, mono_client: MonoClient) -> dict[str, Any]:
    ...
    try:
        ...
        if "error" in balance:
            return {"flow_state": "error", "response": "I couldn't fetch your balance right now."}
        return {
...
async def fetch_affordability(state: QueryState, mono_client: MonoClient) -> dict[str, Any]:
    ...
    try:
        ...
        balance = await mono_client.get_balance(account_id)
        if "error" in balance:
            return {"flow_state": "error", "response": "I couldn't check your balance right now."}
        balance_naira = balance.get("balance_naira", 0)
        ...
        result = {
            ...
            "shortfall": shortfall,
            "remaining": remaining,
            "percentage_of_balance": (amount_check / balance_naira * 100)
            if balance_naira > 0
            else 0,
            "item_name": item_name,
        ...
        if analysis_type == "relative" and avg_daily_spend:
            ...
            result["days_equivalent"] = (
                int(amount_check / avg_daily_spend) if avg_daily_spend > 0 else 0
            )
...
async def fetch_transactions(state: QueryState, mono_client: MonoClient) -> dict[str, Any]:
    ...
    try:
        ...
        return {"flow_state": "error", "response": "I couldn't fetch your transactions right now."}


async def fetch_node(state: QueryState, mono_client: MonoClient) -> dict[str, Any]:
