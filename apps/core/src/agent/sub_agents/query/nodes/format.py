...
async def format_node(state: QueryState, llm: Runnable) -> dict[str, Any]:
...
def _fallback_format(result: dict[str, Any], has_more: bool) -> str:
    ...
    if result_type == "balance":
        ...
        for i, t in enumerate(transactions[:10], 1):
            ...
            lines.append(
                f"{i}. {emoji} {t.get('date', '')} - {t.get('narration', '')[:25]} - ₦{t.get('amount_naira', 0):,.0f}"
            )
        ...
        for day in days:
            lines.append(
                f"• {day['date']}: 📤 ₦{day['spent_naira']:,.0f} | 📥 ₦{day['received_naira']:,.0f}"
            )
        ...
        for i, item in enumerate(items, 1):
            lines.append(
                f"{i}. {item['name']} - ₦{item['total_naira']:,.0f} ({item['count']} txns)"
            )
        return "\n".join(lines)
