...
class ConfirmationNodes:
    ...
    async def confirmation_agent_node(self, state: TransferState) -> TransferState:
        ...
        # Use resolved account name if available (from Flutterwave verification)
        resolved_name = details.get(
            "recipient", {}).get("resolved_account_name")
        display_name = resolved_name or recipient_name or "Recipient"

        bank_name = details.get("recipient", {}).get("bank_name")
        account_number = details.get("recipient", {}).get("account_number")
        source_account_name = details.get(
            "source_account", {}).get("account_name")
...
💸 Amount: ₦{amount:,.2f}
👤 To: {display_name}"""
        if resolved_name and resolved_name != recipient_name:
            confirmation_message += f" ({recipient_name})"
