...
Wraps DataService for workflow execution.
Hybrid preflight: fast param check + capability negotiation.
"""

from shared.types.planner import PlannedTask
from shared.utils.logging import get_logger
from shared.capabilities.data import derive_requirements, decide_capability
from shared.protocols.services import DataServiceProtocol
...
class DataTaskHandler:
    """Handler that executes data purchase tasks via DataService."""
    ...
    async def validate(self, task: PlannedTask, ctx: WorkflowContext) -> TaskResult | None:
        ...
        1. Fast param check (budget/plan, phone)
        2. Capability negotiation
        """
        # --- Layer 1: Fast parameter check ---
        params = task.parameters
        
        budget = params.budget or params.amount
        plan = params.plan
        phone = params.phone or params.recipient_phone
        is_self = params.is_self
        
        missing = []
        if not budget and not plan:
            missing.append("budget or plan")
        ...
        if missing:
            return needs_input_result(
                task.task_id,
                missing,
                f"I need the {' and '.join(missing)} for this data purchase.",
            )
    ...
    async def execute(self, task: PlannedTask, ctx: WorkflowContext) -> TaskResult:
        ...
        try:
            response = await self.service.run_simple(
                ...
                classification_result={"intent": "data", "confidence": 1.0},
            )
            ...
            logger.info("data_task_completed", task_id=task.task_id)
            ...
            logger.exception("data_task_failed", task_id=task.task_id)
