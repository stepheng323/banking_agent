    ...
    async def handle_beneficiary_response(
        ...
        if intent in ("yes", "confirm", "proceed"):
            ...
            if beneficiary_type in ("airtime", "data") and not alias:
                ...
                masked_phone = (
                    f"…{recipient_phone[-4:]}" if len(recipient_phone) >= 4 else recipient_phone
                )
                ...
                with UnitOfWork() as uow:
                    ...
                    if beneficiary_type in ("airtime", "data"):
                        ...
                        if recipient_phone and not network:
                            from apps.core.src.agent.sub_agents.airtime.graph.nodes.extraction import (
                        ...
                        if not network:
                            response = "Network information is required to save this beneficiary."
                            asyncio.create_task(
                                self.context_manager.save_last_response(phone_number, response)
                            )
                            return response
                ...
                try:
                    with UnitOfWork() as uow:
                        ...
                        if beneficiary_type in ("airtime", "data"):
                            ...
                            if not network:
                                response = (
                                    "Network information is required to save this beneficiary."
                                )
                                asyncio.create_task(
                                    self.context_manager.save_last_response(phone_number, response)
                                )
                            ...
                            uow.beneficiaries.create(
                                user_id=str(user.id),
                                beneficiary_type=beneficiary_type,
                                account_name=alias_text[
                                    :64
                                ],  # Use alias as account_name for airtime
                                account_number=recipient_phone,
                                bank_code=None,  # Not used for airtime/data - use NULL instead of empty string
                                bank_name=network,  # Always provided since network is auto-detected
                                alias=alias_text[:64],
                        ...
                        try:
                            ...
                            pass
                        response = (
                            f"✓ Saved as '{alias_text}'. You can now use this alias next time."
                        )
                        asyncio.create_task(
                            self.context_manager.save_last_response(phone_number, response)
                        )
