...
from shared.services.affirmation import AffirmationService, AffirmationResult
...
class AffirmationHandler(MessageHandler):
    ...
    def _get_confirmation_context(self, context: MessageContext) -> Optional[ConfirmationContext]:
    ...
    async def _handle_mandate_reinitiation(
        ...
        if not result.is_approval:
            ...
            return context.with_response(
                "Okay, mandate reinitiation cancelled.",
                handled=True
            )
        
        svc_result = ServiceResult(**await mandate_service.reinitiate_mandate(
