...
def clear_stale_new_task_state(ctx: TransferRunContext, input_state: dict) -> dict:
    ...
    is_new_task_start = is_new_transfer_intent(ctx) and not has_account_number
...
async def clear_old_transfer_values(
    ...
    if is_continuing_flow:
        logger.debug(f"Preserving transfer values - is_continuing_flow={is_continuing_flow}")
        return input_state
...
def clear_recipient_if_needed(ctx: TransferRunContext, input_state: dict) -> dict:
    ...
    should_clear_recipient = (
        is_new_transfer
        and (stale_recipient or stale_bank)
        and current_flow_state not in ("collecting_recipient", "collecting_amount", "confirming")
        and stale_transfer_status not in ("pending", "collection_complete")
        and not has_account_in_message
    )
...
async def prepare_checkpoint_state(
    ...
    should_clear, fresh_state = await check_session_status(ctx, input_state, graph, redis_client)
