...
class BeneficiarySuggestionService:
    ...
    async def check_and_suggest_beneficiary(
        ...
        try:
            with UnitOfWork() as uow:
                ...
                if beneficiary_type == "transfer":
                    ...
                    if not recipient_phone:
                        print(
                            f"DEBUG suggest_beneficiary ({beneficiary_type}): missing phone={recipient_phone}")
                        return
                    ...
                    # Still need network to check if beneficiary exists
                    if not network:
                        return
                    ...
                    if has_beneficiary_repo and not exists_in_beneficiaries:
                        ...
                        # For airtime/data, require alias - update message to make this clear
                        if beneficiary_type in ("airtime", "data"):
                            message = (
                                f"Would you like to save {masked_phone} ({network}) as a beneficiary?\n"
                                f"Please send a name or alias (e.g., 'Mum', 'Home') to save this recipient."
                            )
                        else:
                            message = (
            ...
            print(f"Error checking/suggesting beneficiary: {e}")
            traceback.print_exc()
