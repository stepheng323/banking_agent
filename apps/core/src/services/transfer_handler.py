...
from shared.formatters.receipt import format_transaction_receipt
...
class TransferHandler:
    ...
    async def _send_success_notification(
        ...
        """Send success notification with receipt to user."""
        try:
            ...
            if transaction_record:
                receipt_message = format_transaction_receipt(
                    transaction_record)
                asyncio.create_task(
                    self.whatsapp_client.send_text(
                        to=phone_number, text=receipt_message)
                )
