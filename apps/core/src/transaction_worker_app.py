...
from apps.core.src.lambda_handlers.transaction_worker_handler import _handler as transaction_lambda_handler
...
def _enabled_domain_flags() -> dict[str, bool]:
    return {
        "transaction": settings.enable_transaction_worker,
        "funding": settings.enable_funding_worker,
        "payout": settings.enable_payout_worker,
        "refund": settings.enable_refund_worker,
    }
...
def _enabled_stream_names() -> list[str]:
    topic_flags: dict[TopicType, bool] = {
        "transaction.execute": settings.enable_transaction_worker,
        "funding.process": settings.enable_funding_worker,
        "payout.process": settings.enable_payout_worker,
        "refund.process": settings.enable_refund_worker,
    }
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    any_enabled = any(domain_flags.values())
    all_enabled = all(domain_flags.values())
    worker_enabled = any_enabled and settings.async_transport.lower() in {"aws", "redis"}
    if worker_enabled:
        ...
        if settings.async_transport.lower() == "aws" and any_enabled and not all_enabled:
            reason = "transaction_queue_requires_all_domains_enabled"
        elif not any_enabled:
            reason = "worker_domain_disabled"
        else:
            reason = f"async_transport={settings.async_transport}"
        logger.info(
            "transaction_worker_passive_mode",
            reason=reason,
...
async def readiness_check() -> dict[str, object]:
    """Readiness endpoint exposing active worker-domain ownership."""
    return {
        "status": "ready",
        "service": "transaction-worker",
