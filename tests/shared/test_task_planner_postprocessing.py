...
def test_mixed_source_bank_propagates_to_transfer_when_llm_sets_it_on_airtime_only() -> None:
    planner_output = PlannerOutput(
        primary_intent="mixed",
        tasks=[
            make_planned_task(
    ...
    assert [task.parameters.source_bank_name for task in postprocessed.tasks] == ["GTBank", "GTBank"]


def test_mixed_source_bank_does_not_propagate_without_explicit_source_phrase() -> None:
    planner_output = PlannerOutput(
        primary_intent="mixed",
        tasks=[
            make_planned_task(
    ...
    postprocessed = _postprocess_planner_tasks(
        planner_output,
        "Send 10k to adebayo and buy me 2k airtime",
    )

    assert [task.parameters.source_bank_name for task in postprocessed.tasks] == [None, "GTBank"]
