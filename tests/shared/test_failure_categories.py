...
def test_failure_category_maps_source_and_validation_errors() -> None:
    ...
    assert (
        classify_failure_category(message="source_account_mandate_not_ready", context="execution") == "source_account"
    )
    assert (
        classify_failure_category(message="missing_recipient_account_details", context="execution")
        == "validation_error"
    )
