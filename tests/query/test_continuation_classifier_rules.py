...
def test_next_fact_followup_matches_latest_counterparty_fact_query() -> None:
    query_contract = QueryExecutionContract(
        intent=QueryIntent.TRANSACTION_SEARCH,
        time_start=date(2026, 4, 1),
    ...
    assert is_next_fact_followup("Then who next?", query_contract=query_contract) is True


def test_next_fact_followup_does_not_match_non_latest_fact_query() -> None:
    query_contract = QueryExecutionContract(
        intent=QueryIntent.TRANSACTION_SEARCH,
        time_start=date(2026, 4, 1),
        time_end=date(2026, 4, 10),
