...
def _is_placeholder_group_noun(recipient: str | None, *, language: str | None) -> bool:
    ...
    return all(
        any(_contains_semantic_stem([token], profile["group_nouns"]) for profile in profiles) for token in tokens
    )
...
def _recipient_summary_candidate(
    ...
    *,
    ...
    return has_debit_verb and (
        placeholder_recipient or has_question_cue or (has_group_noun and (has_list_cue or has_ranking_cue))
    )
...
def normalize_month_name_without_year(extraction: QueryExtractionResult) -> QueryExtractionResult:
    ...
    month_names = {period for period, number in MONTH_NAME_TO_NUMBER.items() if number == month_number}
...
def recover_known_fragile_query_shapes(
    ...
    *,
    ...
    if (
        ...
        in {QueryRequestShape.ANALYTICS, QueryRequestShape.COMPARISON, QueryRequestShape.AFFORDABILITY}
        or extraction.fact_query_kind is not None
    ):
        return extraction
...
def normalize_day_scoped_singular_list_query(
    extraction: QueryExtractionResult, *, raw_query: str
) -> QueryExtractionResult:
