...
from banking.support.results import build_receipt_job, result_from_support_response
        ...
        *,
        ...
        if selected:
            (
                ...
                skipped_failed,
                skipped_processing,
                skipped_non_transfer,
            ) = await self._build_receipt_jobs_for_candidates(
                selected=selected,
                candidates=candidates,
                context=context,
                locale=locale,
            ...
            selected_ids = [candidate.transaction_id for candidate in selected]
            served_ids = (
                list(thread_state.served_transaction_ids) if isinstance(thread_state, ReceiptBatchThreadState) else []
            )
            if async_group_id:
